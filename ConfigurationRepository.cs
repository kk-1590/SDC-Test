...
namespace SafeExamBrowser.Configuration
{
	public class ConfigurationRepository : IConfigurationRepository
	{
		...
		{
			return dataValues.LoadDefaultSettings(configurationUrl);
		}
		public AppSettings LoadDefaultSettingsWithSpecifiedUrl(string url)
		{
			return dataValues.LoadDefaultSettings(url);
		}
		...
		public LoadStatus TryLoadSettings(Uri resource, out AppSettings settings, PasswordParameters password = null)
		{
			...
			if (string.IsNullOrWhiteSpace(configurationUrl) && resource.ToString().StartsWith("sdcai:%5C%5C"))
			{
				logger.Info($"SDC-AI Request Found '{resource}'...");
				configurationUrl = resource.ToString().Replace("sdcai:%5C%5C", "");
				logger.Info($"Resource Updated '{configurationUrl}'...");
				resource = new Uri(configurationUrl);
			}
